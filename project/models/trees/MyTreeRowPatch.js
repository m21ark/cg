...
export class MyTreeRowPatch extends CGFobject {
	...
	constructor(scene, coords = [0, 0, 0]) {
		...
		for (let i = 0; i < 6; i++) {
			this.trees.push(new MyBillboard(this.scene, [this.x + i * tree_spacing, this.y, this.z]))
		}
		...
		for (let i = 0; i < this.trees.length; i++) {
			...
			if (rand < 0.5)
				this.trees[i].scale = 9 + Math.random() * 2
			else
				this.trees[i].scale = 10.5 + Math.random() * 3
